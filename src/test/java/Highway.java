...
import static org.testng.Assert.assertEquals;
import static org.testng.Assert.assertTrue;
import static school.redrover.ListOfItems.SHORTS;
    ...
    public class GroupHighwayToAqaTest extends BaseTest {
       ...
       /* Checking the number of items shown on page equals the number depicted in the "show per page" block
        ...
        @Test
        public void testFindListOfItem(ListOfItems listOfItems) throws InterruptedException {
            getDriver().get(BASE_URL);
            WebElement textFieldForSearchItems=getDriver().findElement(By.name("q"));
            textFieldForSearchItems.sendKeys(listOfItems.name());
            Thread.sleep(2000);
